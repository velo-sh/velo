...
impl<'p> IntoPyObject<'p> for FutureResultToPy {
    ...
    fn into_pyobject(self, py: Python<'p>) -> Result<Self::Output, Self::Error> {
        match self {
            ...
            Self::ASGIWSMessage(message) => {
                crate::asgi::conversion::ws_message_into_py(py, message)
            }
            Self::RSGIWSAccept(obj) => obj.into_bound_py_any(py),
            Self::RSGIWSMessage(message) => {
                crate::rsgi::conversion::ws_message_into_py(py, message)
            }
        }
    }
}

pub(crate) fn worker_http1_config_from_py(
    py: Python,
    cfg: Option<Py<PyAny>>,
) -> PyResult<HTTP1Config> {
    let ret = match cfg {
...
pub(crate) fn worker_http2_config_from_py(
    py: Python,
    cfg: Option<Py<PyAny>>,
) -> PyResult<HTTP2Config> {
    let ret = match cfg {
        Some(cfg) => HTTP2Config {
            adaptive_window: cfg.getattr(py, "adaptive_window")?.extract(py)?,
            initial_connection_window_size: cfg
                .getattr(py, "initial_connection_window_size")?
                .extract(py)?,
            initial_stream_window_size: cfg
                .getattr(py, "initial_stream_window_size")?
                .extract(py)?,
            keep_alive_interval: cfg
                ...
                .ok()
                .map(core::time::Duration::from_millis),
            keep_alive_timeout: core::time::Duration::from_secs(
                cfg.getattr(py, "keep_alive_timeout")?.extract(py)?,
            ),
