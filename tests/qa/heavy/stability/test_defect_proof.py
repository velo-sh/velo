...
"""
        ...
        try:
            ...
            assert "LEAK_DETECTED: SENSITIVE_DATA" not in resp.text, (
                f"DEF-72-SEC-001 FAIL: Worker accessed leaked FD {fd}"
            )
...
"""
        ...
        try:
            ...
            print("--- VELO STDOUT ---")
            print(stdout)
            assert "BYPASS_DETECTED" not in resp.text, (
                "DEF-72-SEC-002 FAIL: Worker bypassed EnvironmentShield via PYTHONPATH"
            )
    ...
    def test_DEF_72_SEC_003_symlink_permission_manip(self, isolated_env: VeloTestEnv) -> None:
        ...
        try:
            ...
            assert current_mode != 0o700, (
                f"DEF-72-SEC-003 FAIL: Velo manipulated permissions of symlink target! ({oct(current_mode)})"
            )
        finally:
            proc.terminate()
            proc.wait()
