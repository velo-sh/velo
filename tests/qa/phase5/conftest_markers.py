...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "smoke: L0 Smoke tests - critical path verification"
    )
    config.addinivalue_line(
        "markers", "happy_path: L1 Happy path tests - complete user journey"
    )
    ...
    config.addinivalue_line(
        "markers", "security: L4 Security tests - RFC-0006 §3 requirements"
    )
