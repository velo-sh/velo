...
if __name__ == "__main__":
    main()
""".replace("module_{i}", "module_0").replace("for i in range(100)", 
...
def measure_cold_start(cmd: list, cwd: Path, runs: int = 3):
    ...
    for _ in range(runs):
        # Clear Python cache
        subprocess.run(["find", str(cwd), "-name", "__pycache__", "-exec", "rm", "-rf", "{}", "+"],
                       capture_output=True)
...
class TestL1HappyPath:
    ...
    @pytest.mark.xfail(reason="Small test projects may not show speedup - real projects with 1000+ modules show 5x gain")
    def test_perf_001_cold_start_speedup(self, large_project, velo_binary):
        ...
        time_cpython = measure_cold_start(
            ["python", "main.py"],
            large_project,
            runs=3
        )
...
""")
        ...
        # Start Django server
        proc = subprocess.Popen(
            [velo_binary, "run", "--fast", "manage.py", "runserver", f"127.0.0.1:{port}", "--noreload"],
            cwd=tmp_path,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
