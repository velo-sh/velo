...
""",
            ...
            if '"build_time_ms"' in line:
                try:
                    if line.strip().startswith("{"):
                        ...
                        build_time = json.loads(combined_output[start:end]).get(
                            "build_time_ms", 0
                        )
                    break
        ...
        assert (
            build_time < max_build
        ), f"Build regression: {build_time}ms > {max_build}ms"
        ...
        for line in combined_run.splitlines():
            if '"graph_deserialize_latency_us"' in line:
                try:
                    if line.strip().startswith("{"):
                        latency = json.loads(line).get(
                            "graph_deserialize_latency_us", 999999
                        )
                    else:
                        start = combined_run.find("{", combined_run.find(line))
                        end = combined_run.find("}", start) + 1
                        latency = json.loads(combined_run[start:end]).get(
                            "graph_deserialize_latency_us", 999999
                        )
                    break
                except (json.JSONDecodeError, ValueError):
    ...
    @pytest.mark.xfail(
        reason="P3: Flask metrics output missing graph_deserialize_latency_us (tracked in ARCH-60-001)"
    )
    @pytest.mark.parametrize("scale", ["medium", "large"])
    def test_PERF_603_flask_regression_scale(self, isolated_env, scale):
        ...
        regs = [
            f"from bp{i} import bp as b{i}; app.register_blueprint(b{i})"
            for i in range(n)
        ]
        env.create_app(
            "main.py",
            ...
            if '"graph_deserialize_latency_us"' in line:
                try:
                    if line.strip().startswith("{"):
                        ...
                        latency = json.loads(combined_output[start:end]).get(
                            "graph_deserialize_latency_us", 999999
                        )
                    break
                except (json.JSONDecodeError, ValueError):
    ...
    def test_PERF_604_django_regression_scale(self, isolated_env, scale):
        ...
        env.create_app(
            "settings.py",
            f"SECRET_KEY='f';INSTALLED_APPS={apps};DATABASES={{'default':{{'ENGINE':'django.db.backends.sqlite3','NAME':':memory:'}}}}",
        )
        env.create_app(
            "main.py",
            "import os, django; os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'settings'); django.setup()",
        )
