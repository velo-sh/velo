...
class TestCoreContract:
    ...
    def test_L0_error_invalid_name(self, shm_test_env):
        ...
        try:
            ...
            if "embedded null byte" in str(e):
                pytest.skip(
                    "Python subprocess doesn't support NUL bytes (blocked at test harness level)"
                )
            raise e
    ...
    def test_L0_error_missing_file(self, shm_test_env):
        ...
        result = env.run_velo(
            "analyze", "--shm", "non_existent_file.safetensors", "main.py"
        )
    ...
    def test_L0_cli_shm_flag_missing_analyze(self, shm_test_env):
        ...
        if "--shm" not in result.stdout:
            pytest.fail(
                "REGRESSION: --shm flag DELETED from 'analyze' command! (Finding 004)"
            )
    ...
    def test_L0_cli_shm_flag_missing_run(self, shm_test_env):
        ...
        if "--shm" not in result.stdout:
            pytest.fail(
                "REGRESSION: --shm flag DELETED from 'run' command! (Finding 004)"
            )
    ...
    def test_L0_error_header_too_large(self, shm_test_env):
        ...
        if (
            "HeaderParseFailed" not in result.stderr
            and "InvalidSourceFile" not in result.stderr
        ):
            pytest.fail(
                "REGRESSION: Velo failed to validate safetensors header length (H-22 Violation)"
            )
    ...
    def test_L0_h20_hugepage_erasure(self, shm_test_env):
        ...
        if "MAP_HUGETLB" not in content:
            pytest.fail(
                "REGRESSION: H-20 HugePage support (MAP_HUGETLB) is MISSING from implementation! (Finding 002)"
            )
    ...
    def test_L0_h20_hugepage_integrity(self, shm_test_env):
        ...
        if not has_mfd:
            pytest.fail(
                "REGRESSION: H-20 HugePage support (MFD_HUGETLB) is MISSING! This is a TITANIUM Requirement."
            )
