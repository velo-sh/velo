...
def test_SEC_621_cross_uid_hijack(isolated_env):
    ...
    proc = env.run_velo(
        "serve", "main:app", "--workers", "1",
        env=cmd_env, timeout=10
    )
...
"""
    ...
    proc = subprocess.Popen(
        ...
        cwd=env.path, stderr=subprocess.PIPE, text=True, preexec_fn=os.setsid
    )
    ...
    try:
        ...
        for _ in range(50):
            res = subprocess.run(["curl", "-s", f"http://127.0.0.1:{port}/seed"], 
...
"""
    ...
    try:
        time.sleep(5)
        import json
        res = subprocess.run(["curl", "-s", f"http://127.0.0.1:{port}/fds"], 
                            capture_output=True, text=True)
        data = json.loads(res.stdout)
