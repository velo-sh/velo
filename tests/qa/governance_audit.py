...
class TestGovernanceAlignment(unittest.TestCase):
    ...
    def test_SPEC_0006_naming_prefixes(self):
        ...
        self.assertTrue(
            found_any,
            "🚨 GOVERNANCE FAIL: No formal taxonomy prefixes (core_, bridge_, v_, etc.) found in src/. SPEC-0006 violation.",
        )
    ...
    def test_SPEC_0005_environment_tiers(self):
        ...
        self.assertEqual(
            len(missing), 0, f"🚨 GOVERNANCE FAIL: Missing mandatory environment tiers: {missing}. SPEC-0005 violation."
        )
    ...
    def test_SPEC_0005_audit_tool_presence(self):
        ...
        self.assertIn(
            "velo audit",
            content.lower(),
            "🚨 GOVERNANCE FAIL: 'velo audit' command not found in cli.rs USAGE. SPEC-0005 violation.",
        )
    ...
    def test_SOP_004_zero_hardcode_clean_sweep(self):
        ...
        if config_toml.exists():
            ...
            toxic_patterns = [r'"/Users/[^"]+"', r'"/home/[^"]+"', r'"/private/var/[^"]+"', r'"/tmp/[^"]+"']
            for pattern in toxic_patterns:
                matches = re.findall(pattern, content)
                self.assertEqual(
                    len(matches),
                    0,
                    f"🚨 GOVERNANCE FAIL: Hardcoded absolute path '{matches}' found in constants.toml. SOP-004 violation.",
                )
        ...
        self.assertEqual(
            len(found_toxins),
            0,
            f"🚨 GOVERNANCE FAIL: Absolute path toxins found in src/: {found_toxins}. SOP-004 violation.",
        )
