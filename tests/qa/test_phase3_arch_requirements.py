...
class TestMemoryUsage:
    ...
    def test_mem_002_worker_cow_efficiency(self):
        ...
        try:
            ...
            env.create_script("mem_report.py", """
import os
import sys
...
""")
            ...
            for _ in range(10):
                t = threading.Thread(
                    target=lambda: run_velo(["run", "--zygote", "hold.py"], cwd=env.path, timeout=30)
                )
                threads.append(t)
                t.start()
...
""")
            ...
            env.create_script("use_custom.py", """
