...
"""
    ...
    def test_native_sovereignty_uvicorn_absence(self, isolated_env, run_velo_e2e):
        ...
        try:
            ...
            assert "UVICORN_LOADED:False" in body, f"SOVEREIGNTY VIOLATION: uvicorn pre-loaded in RSGI mode! Response: {body}"
    ...
    def test_signal_zombie_cleanup(self, isolated_env, run_velo_e2e):
        ...
        try:
            ...
            assert len(worker_pids) >= 3, f"Expected at least 3 child processes (Zygote + 2 workers), found {len(worker_pids)}"
            ...
            if failed:
                ...
                if stdout_p.exists():
                    with open(stdout_p, "r") as f:
                        print(f"--- Velo Stdout ---\n{f.read()}\n------------------")
                if stderr_p.exists():
                    with open(stderr_p, "r") as f:
                        print(f"--- Velo Stderr ---\n{f.read()}\n------------------")
                if debug_log.exists():
