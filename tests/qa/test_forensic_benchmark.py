...
class TestVeloForensic:
    ...
    def test_forensic_comparison(self, count):
        ...
        duration, output = run_bench([sys.executable, "-m", "pytest", str(test_dir), "-q", "--no-header"], env, "Standard Single Process")
        ...
        bust_cache(project_dir)
        duration_velo, output_velo = run_bench(["pytest", str(test_dir), "--velo", "-q", "--no-header"], env, "Velo Zygote")
        ...
        print("-" * 50)
        print(f"| Mode                 | Wall-Clock Time | Rel. Speed |")
        print(f"| :------------------- | :-------------- | :--------- |")
        print(f"| Subprocess-Isolated  | {results['Subprocess-Isolated']:>14.2f}s | 1.0x (Ref) |")
        print(f"| Single-Process       | {results['Single-Process']:>14.2f}s | {results['Subprocess-Isolated']/results['Single-Process']:>9.1f}x |")
        print(f"| **Velo-Isolated**    | {results['Velo-Isolated']:>14.2f}s | {results['Subprocess-Isolated']/results['Velo-Isolated']:>9.1f}x |")
        print("-" * 50)
