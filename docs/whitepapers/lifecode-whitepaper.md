...
### 6.1 Metabolism: The Genotype (Seed) vs. The Phenotype (Growth)
We correct the definition of Existence: **Identity is Potential; Life is Actualization.**

*   **The Genotype (The Hash)**:
    *   This is the **Seed**. It contains the immutable Code, Assets, and Interface Definitions ("I need a database").
    *   It **strictly excludes** Environment (Config, Secrets, Peers).
    *   *Law*: `Hash = CompilerOutput`. It is a pure, mathematical entity.
*   **The Phenotype (The Runtime)**:
    *   This is the **Plant**. It is the result of `Seed + Soil`.
    *   **Metabolism** is the interaction of the Seed with the **Environment** (Soil).
    *   *Law*: `Life = Hash(Seed) + Runtime(State)`.
*   **Implication**: You can move the Seed (deploy), but you must tend the Soil (ops). The Seed guarantees what *can* happen; the Soil determines what *does* happen.
