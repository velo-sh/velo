...
mod tests {
    ...
    // ========================================================================
    // New ServeArgs fields tests (D1 - CLI args)
    // ========================================================================

    #[test]
    fn test_serve_args_timeout_default() {
        let args = ServeArgs::default();
        assert_eq!(args.timeout, 30); // Default timeout is 30 seconds
    }

    #[test]
    fn test_serve_args_custom_timeout() {
    ...
    #[test]
    fn test_serve_args_health_bind_none_by_default() {
        let args = ServeArgs::default();
        assert!(args.health_bind.is_none());
    }

    #[test]
    fn test_serve_args_pid_file_none_by_default() {
        let args = ServeArgs::default();
        assert!(args.pid_file.is_none());
    }

    #[test]
    fn test_serve_args_log_format_default_text() {
        let args = ServeArgs::default();
        assert!(matches!(args.log_format, LogFormat::Text));
    }

    #[test]
    fn test_serve_args_prod_mode_disabled_by_default() {
        let args = ServeArgs::default();
        assert!(!args.prod);
    }
    ...
    #[test]
    fn test_apply_prod_mode_disables_reload() {
        let mut args = ServeArgs::default();
        args.reload = true;
        args.prod = true;
        args.apply_prod_mode();

        assert!(!args.reload, "Reload should be disabled in prod mode");
    }

    #[test]
    fn test_apply_prod_mode_auto_sets_workers() {
        let mut args = ServeArgs::default();
        args.workers = 1;
        args.prod = true;
        args.apply_prod_mode();

        // Should auto-set workers to CPU count (at least 1)
        assert!(args.workers >= 1, "Workers should be auto-set in prod mode");
    }

    #[test]
    fn test_apply_prod_mode_preserves_explicit_workers() {
        let mut args = ServeArgs::default();
        args.workers = 8; // Explicitly set
        args.prod = true;
        args.apply_prod_mode();

        assert_eq!(args.workers, 8, "Explicit workers should be preserved");
    }

    #[test]
    fn test_apply_prod_mode_noop_when_disabled() {
        let mut args = ServeArgs::default();
        args.reload = true;
        args.workers = 1;
        args.prod = false;
        ...
        assert!(
            args.reload,
            "Reload should remain unchanged when prod=false"
        );
        assert_eq!(
            args.workers, 1,
            "Workers should remain unchanged when prod=false"
        );
    }
    ...
    #[test]
    fn test_log_format_text_variant() {
        let format = LogFormat::Text;
        assert!(matches!(format, LogFormat::Text));
    }

    #[test]
    fn test_log_format_json_variant() {
