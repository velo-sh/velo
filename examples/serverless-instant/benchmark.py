...
def run_benchmark(scenario: str = "single", runs: int = 5) -> BenchmarkResult:
    ...
    """
    from cpython_runner import run_batch as cpython_batch
    from cpython_runner import run_single as cpython_single
    from velo_runner import VeloZygote

    print(f"\n{'=' * 60}")
    print(" HIO-004 SERVERLESS BENCHMARK")
...
def print_results(result: BenchmarkResult, scenario: str):
    ...
    # Calculate p95
    cpython_p95 = (
        sorted(result.cpython_times)[int(len(result.cpython_times) * 0.95)]
        if len(result.cpython_times) >= 5
        else max(result.cpython_times)
    )
    velo_p95 = (
        sorted(result.velo_times)[int(len(result.velo_times) * 0.95)]
        if len(result.velo_times) >= 5
        else max(result.velo_times)
    )
