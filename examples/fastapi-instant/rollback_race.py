...
def wait_for_port(port: int, timeout: float = 5.0) -> bool:
    ...
    while time.time() - start < timeout:
        try:
            ...
            subprocess.run(["nc", "-z", "127.0.0.1", str(port)], check=True, capture_output=True, timeout=0.5)
            return True
...
def get_process_rss(pid: int) -> float:
    ...
    try:
        if sys.platform == "darwin":
            result = subprocess.run(["ps", "-o", "rss=", "-p", str(pid)], capture_output=True, text=True, timeout=1.0)
...
def main():
    ...
    try:
        with progress:
            ...
            trad_time, trad_rss, per_proc, trad_times = measure_traditional_resets(
                n, workspace, progress, trad_task, is_rich
            )
        ...
        print_race_result(
            c_median, v_median, mode=f"Environment Reset (Median of {n} runs)", memory_data=(trad_rss, velo_rss)
        )
