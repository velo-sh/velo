    ...
    if sys.platform == "darwin":
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                result = subprocess.run(["sysctl", "-n", "hw.model"], capture_output=True, text=True, timeout=2)
...
def get_velo_version() -> str:
    ...
    try:
        ...
        result = subprocess.run([str(velo_bin), "--version"], capture_output=True, text=True, timeout=2)
...
def print_lab_environment():
    ...
    if RICH_AVAILABLE:
        ...
        console.print(Panel("\n".join(info_lines), title="[bold white]LAB ENVIRONMENT[/]", border_style="cyan"))
    else:
        print("=" * 60)
...
def print_race_result(
    cpython_time: float, velo_time: float, mode: str = "Warm Cache", memory_data: Optional[Tuple] = None
):
    ...
    if IS_QUIET:
        ...
        print(
            f"CPython: {format_time(cpython_time)} | Velo: {format_time(velo_time)} | Speedup: {cpython_time / max(velo_time, 0.0001):.1f}x{mem_str}"
        )
        return
    ...
    if RICH_AVAILABLE:
        table = Table(title=f"🏁 BENCHMARK RACE [dim](Mode: {mode})[/]", show_header=True, border_style="cyan")
...
def print_verdict(speedup: float, mem_reduction: float = 0):
    ...
    summary_text = (
        f"[bold green]SUMMARY:[/] Velo is [bold yellow]{speedup:.2f}x[/] faster than traditional CPython cold starts."
    )
    if mem_reduction > 0:
        summary_text += f"\n[dim]Memory saved: {mem_reduction * 100:.0f}%[/]"

    if RICH_AVAILABLE:
        console.print(Panel(summary_text, title="[bold white]Verdict[/]", border_style="green"))
    else:
        print("-" * 60)
        print(f" SUMMARY: Velo is {speedup:.2f}x faster")
        if mem_reduction > 0:
...
def export_results_json(
    ...
    results = {
        "results": [
            {"name": cpython_label, "times": cpython_times, "mean": cp_mean},
            {"name": velo_label, "times": velo_times, "mean": ve_mean},
        ],
...
def save_summary_metric(
    ...
    if cpython_time is not None and velo_time is not None:
        save_rich_report(name, cpython_time, velo_time, cpython_rss or 0.0, velo_rss or 0.0)


def save_rich_report(name: str, cpython_time: float, velo_time: float, cpython_rss: float = 0.0, velo_rss: float = 0.0):
    """
