...
class EnterpriseGenerator:
    ...
    def create_fastapi(path: Path, modules=500):
        ...
        for i in range(10): # 10 model files
            ...
            for j in range(modules // 10):
                code.append(f"class Model_{i}_{j}(BaseModel):\n    id: int\n    name: str\n    meta: Optional[dict] = None")
        ...
        routers_dir.mkdir()
        for i in range(20):
            code = [f"from fastapi import APIRouter\nfrom ..models.m{i % 10} import Model_{i % 10}_0", f"router = APIRouter()"]
            code.append(f"@router.get('/{i}')\ndef route_{i}(): return Model_{i % 10}_0(id={i}, name='test')")
            (routers_dir / f"r{i}.py").write_text("\n".join(code))
            
        # 3. Main Entry
        main_code = ["from fastapi import FastAPI", "from .internal import " + ", ".join([f"r{i}" for i in range(20)])]
    ...
    def create_django(path: Path, apps=50):
        ...
        for i in range(apps):
            ...
            (app_dir / "views.py").write_text(f"from django.http import HttpResponse\nfrom .models import Model_{i}\ndef v_{i}(r): return HttpResponse('ok')")
            installed_apps.append(app_name)

        # 2. Settings
        (path / "settings.py").write_text(f"""
SECRET_KEY = 'secret'
