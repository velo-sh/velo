...
SCALE_LEVELS = {
    ...
    "L2": {"name": "Small App", "models": 10, "routes": 10, "apps": 5, "blueprints": 10},
    "L3": {"name": "Medium App", "models": 50, "routes": 50, "apps": 20, "blueprints": 50},
    "L4": {"name": "Large App", "models": 200, "routes": 100, "apps": 50, "blueprints": 100},
    "L5": {"name": "Enterprise", "models": 500, "routes": 200, "apps": 100, "blueprints": 200},
}
...
def generate_flask_project(project_dir: Path, level: str) -> int:
    ...
    for i in range(n_blueprints):
        (bp_dir / f"bp_{i}.py").write_text(f'''
...
def generate_django_project(project_dir: Path, level: str) -> int:
    ...
    (project_dir / "settings.py").write_text(f'''
        ...
        try:
            ...
            if build_result.returncode != 0:
                return BenchmarkResult(
                    framework=framework, level=level, scale_name=config["name"],
                    components=components, build_time_ms=build_time_ms, load_time_ms=0,
                    success=False, error=f"Build failed: {build_result.stderr[:200]}"
                )
            ...
            for _ in range(5):
                ...
                if expected_output not in run_result.stdout:
                    return BenchmarkResult(
                        framework=framework, level=level, scale_name=config["name"],
                        components=components, build_time_ms=build_time_ms, load_time_ms=run_time_ms,
                        success=False, error=f"Run failed or wrong output. Got: {run_result.stdout[:100]}"
                    )
            ...
            return BenchmarkResult(
                framework=framework, level=level, scale_name=config["name"],
                components=components, build_time_ms=build_time_ms, load_time_ms=avg_load_time,
                success=True
            )
            ...
            return BenchmarkResult(
                framework=framework, level=level, scale_name=config["name"],
                components=0, build_time_ms=0, load_time_ms=0,
...
def print_results(results: list[BenchmarkResult]):
    ...
    print("=" * 90)
    print(f"{'Framework':<12} {'Level':<5} {'Scale':<15} {'Components':>10} {'Build (ms)':>12} {'Load (ms)':>12} {'Status':<8}")
    print("-" * 90)
    
    for r in results:
        status = "✅ PASS" if r.success else "❌ FAIL"
        if r.success:
            print(f"{r.framework:<12} {r.level:<5} {r.scale_name:<15} {r.components:>10} {r.build_time_ms:>12.1f} {r.load_time_ms:>12.1f} {status:<8}")
        else:
            print(f"{r.framework:<12} {r.level:<5} {r.scale_name:<15} {r.components:>10} {'--':>12} {'--':>12} {status:<8}")
...
def main():
    ...
    if args.all:
        ...
        if args.fastapi: frameworks.append("fastapi")
        if args.flask: frameworks.append("flask")
        if args.django: frameworks.append("django")
