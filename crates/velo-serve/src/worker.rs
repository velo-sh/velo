...
use uuid::Uuid;
use velo_core::zygote::core_ipc as ipc;
...
fn build_worker_env(
    ...
    let shield = velo_core::lifecycle::v_shield::EnvironmentShield::new(config);
...
impl Worker {
    ...
    pub fn spawn_uds_via_zygote(
        ...
        let launcher_path = velo_core::zygote::find_worker_launcher(config)
            .map_err(|e| anyhow::anyhow!("Zygote launcher error: {}", e))?;

        // 1. Determine a unique UDS path (Industrial Grade - Standardized)
        let socket_path = velo_core::common::paths::generate_worker_socket_path(worker_id);
    ...
    pub fn spawn_native(
        ...
        let shield = velo_core::lifecycle::v_shield::EnvironmentShield::new(_config);
        ...
        match velo_core::common::python_env::PythonEnv::detect(python_path) {
            Ok(py_env) => {
