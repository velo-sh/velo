...
fn check_python_environment(project_dir: &std::path::Path) -> PythonInfo {
    let python_path =
        velo_core::python::detect_python(project_dir).unwrap_or_else(|_| "python3".into());
    let hermetic = velo_core::python::is_hermetic_check(&python_path);
        ...
        && o.status.success()
    {
        ...
        if version == velo_core::common::constants::PYTHON_VERSION && hermetic {
            ok = true;
        }
    }
