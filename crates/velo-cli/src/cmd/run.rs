...
use velo_core::cache::EnvCache;
use velo_core::common::diagnostics::{MarkdownFormatter, StartupTimeline};
pub use velo_core::common::paths::*;
use velo_core::config::VeloConfig;
use velo_core::custody::{AutopilotDecision, AutopilotEngine, EnvironmentSync};
...
fn run_vibe_mode(cmd: &RunCmd) -> Result<()> {
...
fn run_script_impl(cmd: &RunCmd) -> Result<()> {
            ...
            match registry.create_segment(&segment_name, shm_path) {
                ...
                Err(e) => {
                    let signal = velo_core::common::governance::GovernanceSignal::new(
                        velo_core::common::governance::SignalComponent::MemoryGravity,
    ...
    let (pythonpath, needs_capture) =
        velo_core::python::setup_python_env(&project_dir, &python_path);

    // AI-Native Diagnostics (RFC-0038)
    if let Some(prof_md_path) = &cmd.prof_md {
        let (status, total_time, profile_data) =
            velo_core::runner::run_script_with_profile_capture(
                &python_path,
                script_path_str,
                pythonpath.clone(),
                &config,
    ...
    if cmd.fast {
        if let Err(e) = run_with_fast_loader(
            ...
            let signal = velo_core::common::governance::GovernanceSignal::new(
                velo_core::common::governance::SignalComponent::FastLoader,
            ...
            if config.strict_optimizations {
                ...
                signal.report_audit();
                velo_core::runner::run_script(&python_path, script_path_str, None, &config)?;
            }
        }
    } else if cmd.profile {
        velo_core::runner::run_script_with_profile(
            &python_path,
            script_path_str,
            pythonpath,
            &config,
        ...
        velo_core::runner::run_script(&python_path, script_path_str, pythonpath, &config)?;
    }
...
fn run_module_impl(cmd: &RunCmd) -> Result<()> {
    ...
    use velo_core::lifecycle::{EnvironmentShield, apply_standard_hygiene};
    ...
    if let Some(prof_md_path) = &cmd.prof_md {
        let (status, total_time, profile_data) =
            velo_core::runner::run_module_with_profile_capture(
                &PathBuf::from(&python_path),
                module_name,
                &all_args,
                &config,
...
fn try_zygote_run(
    ...
    if !velo_core::zygote::is_supported() {
        return Ok(None);
    }
...
fn save_cache_if_needed(project_dir: &Path, python_path: &Path) {
    ...
    // Capture sys.path
    match velo_core::python::capture_sys_path(python_path) {
        Ok(syspath) => {
    ...
    let res = (|| -> Result<()> {
        ...
        let actual_bundle = match find_bundle(project_dir, script_path) {
            ...
            None => {
                ...
                eprintln!("   Falling back to normal mode...");
                return velo_core::runner::run_script(python_path, script_path, pythonpath, config);
            }
        };

        // RFC-0008: Mandatory Security Pre-validation (H-1, H-2, H-4, H-5)
        // This provides a structural lock at the Rust boundary.
        if let Err(e) = velo_core::loader::verify::load_and_verify(&actual_bundle, max_bundle_size)
        {
...
"#,
        ...
        velo_core::runner::run_script(python_path, script_path, Some(enhanced_pythonpath), config)
    })();

    // Always report metrics, even on error
    velo_core::graph::report_metrics();
    res
}
